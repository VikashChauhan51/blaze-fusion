...
/// </summary>
public abstract class BlazeComponent : TagHelper, IViewContextAware
{
    ...
    private readonly List<BlazeComponentEvent> _dispatchEvents = new();
    private readonly HashSet<BlazeEventSubscription> _subscriptions = new();
    ...
    /// <summary />
    public BlazeComponent()
    {
        Subscribe<BlazeBind>(data => SetPropertyValue(data.Name, data.Value));
        ConfigurePolls();
        Client = new BlazeClientActions(this);
    }
    ...
    private void ConfigurePolls()
    {
        ...
        var methods = componentType
            ...
            .Where(m => m.DeclaringType != typeof(BlazeComponent))
        ...
        var polls = methods
            .Select(p => new BlazePoll(p.Method.Name, TimeSpan.FromMilliseconds(p.Attribute.Interval)))
            .ToList();
    ...
    /// </summary>
    public BlazeClientActions Client { get; private set; }
    ...
    public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
    {
        ...
        _options = services.GetService<BlazeOptions>();
        _logger = services.GetService<ILogger<BlazeComponent>>() ?? NullLogger<BlazeComponent>.Instance;

        if (_persistentState == null || _options == null)
        {
            throw new ApplicationException("Blaze has not been initialized with UseBlaze in the application startup.");
        }
        ...
        try
        {
            var componentHtml = HttpContext.IsBlaze(excludeBoosted: true)
    ...
    {
        var message = $"There was a problem with rendering Blaze component {GetType().Name}";
        _logger.LogError(e, message);
        Dispatch(new UnhandledBlazeError(message, e), Scope.Global);
    }

    /// <summary>
    /// Subscribes to a Blaze event
    /// </summary>
    /// <typeparam name="TEvent">Event type</typeparam>
    public void Subscribe<TEvent>(Func<string> subject = null) =>
        _subscriptions.Add(new BlazeEventSubscription
        {
    ...
    {
        var operationId = !asynchronous && HttpContext.Request.Headers.TryGetValue(BlazeConsts.RequestHeaders.OperationId, out var incomingOperationId)
            ? incomingOperationId.First()
            : Guid.NewGuid().ToString("N");

        _dispatchEvents.Add(new BlazeComponentEvent
        {
            Name = name,
            Data = data,
    ...
    /// <summary>
    /// Triggers a Blaze event in a global scope
    /// </summary>
    /// <param name="data">Data to pass</param>
    /// <param name="subject">Subject</param>
    /// <param name="asynchronous">Do not chain the execution of handlers and run them separately</param>
    ...
    public void Redirect(string url) =>
        HttpContext.Response.BlazeRedirect(url);
    ...
    public void Location(string url, object payload = null) =>
        HttpContext.Response.BlazeLocation(url, payload);
    ...
    {
        if (!HttpContext.Items.TryGetValue(BlazeConsts.ContextItems.IsRootRendered, out _))
        {
            HttpContext.Items.TryAdd(BlazeConsts.ContextItems.IsRootRendered, true);
            return true;
        }
    ...
    {
        var parentComponentId = HttpContext.Items[BlazeConsts.Component.ParentComponentId];
    ...
    {
        var previousParentComponentId = HttpContext.Items[BlazeConsts.Component.ParentComponentId];
        HttpContext.Items[BlazeConsts.Component.ParentComponentId] = componentId;

        var componentHtmlDocument = await GetComponentHtml();

        HttpContext.Items[BlazeConsts.Component.ParentComponentId] = previousParentComponentId;
        ...
        if (includeScripts)
        {
            ...
            {
                AddClientScript($"Blaze.dispatchEvents('{GetSerializedEventDispatchers()}', this);");
            }
    ...
    private async Task BindModel(IFormCollection formCollection)
    {
        if (!IsModelTouched)
        {
            if (HttpContext.Items.TryGetValue(BlazeConsts.ContextItems.MethodName, out _))
            {
                IsModelTouched = true;
            }
            else
    ...
    private HtmlNode GetEventSubscriptionScript(HtmlDocument document, BlazeEventSubscription subscription)
    {
        var eventData = new
        {
    ...
    private void PopulateDispatchers()
    {
        ...
        HttpContext.Response.Headers.TryAdd(BlazeConsts.ResponseHeaders.Trigger, GetSerializedEventDispatchers());
    }
    ...
    private void PopulateClientScripts()
    {
        ...
        HttpContext.Response.Headers.TryAdd(BlazeConsts.ResponseHeaders.Scripts, Base64.Serialize(_clientScripts));
    ...
    {
        var renderedComponentIds = (string[])HttpContext.Items[BlazeConsts.ContextItems.RenderedComponentIds];
        return renderedComponentIds.Contains(componentId);
    }

    private async Task TriggerMethod()
    {
        if (!HttpContext.Items.TryGetValue(BlazeConsts.ContextItems.MethodName, out var method)
        ...
        var requestParameters = (IDictionary<string, object>)HttpContext.Items[BlazeConsts.ContextItems.Parameters];
        ...
        {
            _skipOutput = true;
            HttpContext.Response.Headers.TryAdd(BlazeConsts.ResponseHeaders.SkipOutput, "True");
        }

        var operationId = HttpContext.Request.Headers.TryGetValue(BlazeConsts.RequestHeaders.OperationId, out var incomingOperationId)
            ? incomingOperationId.First()
            : Guid.NewGuid().ToString("N");

        HttpContext.Response.Headers.TryAdd(BlazeConsts.ResponseHeaders.OperationId, operationId);
    ...
    {
        if (!HttpContext.Items.TryGetValue(BlazeConsts.ContextItems.EventName, out var eventName)
            || eventName is not string eventNameValue || string.IsNullOrWhiteSpace(eventNameValue))
        {
            return;
        }

        var eventSubject = (string)HttpContext.Items[BlazeConsts.ContextItems.EventSubject];
        ...
        foreach (var subscription in subscriptions)
        {
            ...
            var model = Base64.Deserialize((string)HttpContext.Items[BlazeConsts.ContextItems.EventData], parameterType);
    ...
    {
        if (HttpContext.Items.TryGetValue(BlazeConsts.ContextItems.RequestForm, out var requestForm))
        {
    ...
    private string GetRootComponentId() =>
        ((string[])HttpContext.Items[BlazeConsts.ContextItems.RenderedComponentIds]).First();
    ...
    private async Task<bool> AuthorizeAsync()
    {
        ...
        if (!ComponentAuthorizationAttributes.ContainsKey(type))
        {
            ComponentAuthorizationAttributes.TryAdd(type, type.GetCustomAttributes(true)
                .Where(attr => attr is IBlazeAuthorizationFilter)
                .Cast<IBlazeAuthorizationFilter>()
                .ToArray());
        }
        ...
        foreach (var authorizationFilter in ComponentAuthorizationAttributes[type])
        {
            ...
            {
                if (HttpContext.IsBlaze(excludeBoosted: true))
                {
