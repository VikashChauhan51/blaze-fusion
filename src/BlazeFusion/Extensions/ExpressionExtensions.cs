...
{
    private const string JsIndicationStart = "BLAZE_JS(";
    private const string JsIndicationEnd = ")BLAZE_JS";
    ...
    public static (string Name, IDictionary<string, object> Parameters)? GetNameAndParameters(this LambdaExpression expression)
    {
        ...
        for (var i = 0; i < arguments.Count; i++)
        {
            ...
            {
                throw new NotSupportedException($"Unsupported expression type in the Blaze action call: {expression.GetType().Name}, parameter: {paramName}. Try to use primitive value as a parameter.", exception);
            }
        }
