function BlazeCore() {
  ...
  const configMeta = document.querySelector('meta[name="blaze-config"]');
  ...
  function createScriptTag(content, src, autoRemove) {
    ...
    script.setAttribute('blaze-loaded', 'true');
    return script;
  }
  ...
  function enablePlainScripts(element) {
    enableScripts(element, 'script[type="text/javascript"]:not([blaze-loaded]):not([blaze-js])');
  }

  function enableblazeScripts(componentElement) {
    enableScripts(componentElement, `script[blaze-js][blaze-id="${componentElement.id}"]:not([blaze-loaded])`);
  }
  ...
  async function loadPageContent(url, push, condition, payload) {
    document.body.classList.add('blaze-loading');
    ...
    try {
      ...
      document.body.classList.remove('blaze-loading');
    }
  }

  function enqueueblazePromise(key, promiseFunc) {
    if (!promiseChain[key]) {
  ...
  function findComponent(el) {
    const component = el.closest("[blaze]");
    ...
    return {
      ...
      name: component.getAttribute("blaze-name"),
      element: component
    };
  }
  ...
  async function blazeEvent(el, blazeEventRaw, url, eventData) {
    const operationId = eventData.operationId;
    const blazeEvent = JSON.parse(blazeEventRaw);
    el.setAttribute("data-operation-id", operationId);

    await blazeRequest(el, url, { eventData: { name: blazeEvent.name, data: eventData.data, subject: eventData.subject } }, 'event', blazeEvent, operationId);
  }

  function blazeDispatch(el, event, operationId) {
  ...
  async function blazeBind(el) {
    ...
    el.classList.add('blaze-request');
    ...
    binding[component.id].promise = new Promise(resolve => {
      binding[component.id].timeout = setTimeout(async () => {
        ...
        await blazeRequest(el, url, { formData: requestFormData }, 'bind', null, bindOperationId);
  ...
  async function blazeAction(el, component, action) {
    const url = `/blaze/${component.name}/${action.name}`;

    if (Array.from(el.attributes).some(attr => attr.name.startsWith('x-blaze-bind')) && isElementDirty(el)) {
      await blazeBind(el);
    }

    if (document.activeElement && document.activeElement !== el) {
      const elToCheck = document.activeElement;
      if (Array.from(elToCheck.attributes).some(attr => attr.name.startsWith('x-blaze-bind')) && isElementDirty(elToCheck)) {
        await blazeBind(elToCheck);
      }
    }
    ...
    await blazeRequest(el, url, { parameters: action.parameters }, 'action', null, operationId, true);
  }

  let operationStatus = {};

  async function blazeRequest(el, url, requestData, type, eventData, operationId, morphActiveElement) {
    ...
    if (operationId) {
      ...
      if (el.getAttribute('data-operation-id') === operationId) {
        classTimeout = setTimeout(() => el.classList.add('blaze-request'), 200);
    ...
    await enqueueblazePromise(componentId, async () => {
      try {
        ...
        requestForm.append('__blaze_type', type);

        if (requestData.parameters) {
          const paramsData = JSON.stringify(requestData.parameters, (k, v) => v === undefined ? null : v);
          requestForm.append('__blaze_parameters', paramsData)
        }

        if (requestData.eventData) {
          requestForm.append('__blaze_event', JSON.stringify(requestData.eventData))
        }
        ...
        requestForm.append('__blaze_componentIds', JSON.stringify([componentId, ...dataIds]));
          ...
          if (!skipOutputHeader) {
            ...
            Alpine.morph(component, responseData, {
              updating: (from, to, childrenOnly, skip) => {
                if (counter !== 0 && to.getAttribute && to.getAttribute("blaze") !== null && from.getAttribute && from.getAttribute("blaze") !== null) {
                  if (to.getAttribute("blaze-placeholder") !== null) {
                    // blaze component placeholder, so skipping
                    skip();
                    counter++;
                    return;
                  }
                ...
                if (from.getAttribute && from.getAttribute("data-operation-id")) {
                  // skip result from bind operation when this operating element is not a blaze component and is awaiting a request already
                  if (type === 'bind' && operationId !== from.getAttribute("data-operation-id") && from.getAttribute("blaze") === null) {
                    skip();
                    counter++;
                    return;
                  }

                  // set the operation id, disabled state and blaze class that would be lost after morph
                  to.setAttribute("data-operation-id", from.getAttribute("data-operation-id"));
                  to.disabled = from.disabled;
                  if (from.classList.contains('blaze-request')) {
                    to.classList.add('blaze-request');
                  }
                }
          ...
          if (operationId) {
            ...
            if (operationTrigger.length && (operationStatus[operationId] <= 0)) {
              operationTrigger.forEach(trigger => {
                ...
                trigger.classList.remove('blaze-request');
...
window.Blaze = new BlazeCore();

document.addEventListener('alpine:init', () => {
  Alpine.directive('blaze-dispatch', (el, { expression }, { effect, cleanup }) => {
    effect(() => {
      if (!document.contains(el)) {
        return;
      }
  ...
  Alpine.directive('blaze-bind', (el, { value, modifiers }, { effect, cleanup }) => {
    effect(() => {
      const event = value;
  ...
  Alpine.directive('blaze-link', (el, { expression }, { effect, cleanup }) => {
    effect(() => {
  ...
  Alpine.directive('blaze-focus', (el, { expression }, { effect }) => {
    effect(() => {
