...
public sealed class BlazeBindShorthandTagHelper : TagHelper
{
    ...
    public override void Process(TagHelperContext context, TagHelperOutput output)
    {
        ...
        if (DefaultEvent)
        {
            output.Attributes.Add(new("x-blaze-bind:change"));
        }
        ...
        if (_events != null)
        {
            foreach (var eventItem in _events)
            {
                ...
                output.Attributes.Add(new($"x-blaze-bind:{eventDefinition}"));
            }
        }
    }
}
